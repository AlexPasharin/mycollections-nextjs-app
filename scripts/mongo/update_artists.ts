...
Promise.all([
  ...
  async ([dbArtists, dbEntries, dbEntryTypes, dbReleases, mongoArtists]) => {
    ...
    const artists = dbArtists.reduce<EnhancedArtist[]>((acc, artist) => {
      ...
      const entriesGroupedByType = pipe(
          ...
          pipe(
            (entries) =>
              entries.map(({ entry_artist_id, ...rest }) => {
                const strippedEntry: Entry = removeNulls(
                  omit(["type", "artist_id"], rest)
                );
                ...
                const releases = releasesGroupedByEntries[rest.id];

                if (entryArtist) {
                  strippedEntry.entryArtist = entryArtist;
                }

                if (releases) {
                  strippedEntry.releases = releases.map(removeNulls);
                }

                return strippedEntry;

                // TODO: fix types in this, more functional, approach

                // return removeNulls({
                //   ...strippedEntry,
                //   entryArtist,
                //   releases: releases
                //     ? releases.map(removeNulls<DBRelease>)
                //     : null,
                // });
              }),
        ...
        toPairs<Record<string, Entry[]>, string>,
        sortBy(prop(0)),
        fromPairs
      )(entries);
   ...
   */
    const artistsOnlyInMongo = mongoArtists.filter(
      ({ _id }) => !artistsMap[_id]
    );
    ...
    const artistsToUpdate = artists.filter((artist) => {
      ...
      return (
        !mongoArtist ||
        !equals(omit(["id"], artist), omit(["_id"], mongoArtist))
      );
    });
